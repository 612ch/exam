					...
					<c:choose>
						<c:when test="${fn:length(ehps) > 0 }">
							<c:forEach items="${ehps }" var="ehp">
								<div class="col-md-4">
									<div class="thumbnail">
										<img alt="300x200" src="${path}/images/reception/peitu2.jpg" />
										<div class="caption">
											<h3>
												${ehp.examPaperName }
											</h3>
											...
											<p>
									 			<a class="btn btn-default btn-lg btn-block beginExam" href="${path}/Exam/review.action?studentId=${student.studentId }&examPaperId=${ehp.examPaperId }&score=${ehp.examScore }&examPaperName=${ehp.examPaperName }">回顾试卷</a>
											</p>
										</div>
									</div>
								</div>
