...
$(function() {
	...
	$(".lookclass").click(function() {
        ...
        $.ajax({
            type:"POST",
            url:path+"/class/getClassByGradeId.action",
	...
	$(".examPaper-subjects").click(function() {
		...
		$("#afterTr").children("td").append("<table class='table' style='width:100%;'>" +
				...
				"</table>");
        //提交请求获取班级数据
        var examPaperId=$(this).attr("id");
       /* $.ajax({
            type:"POST",
            url:path+"/examPaper/addSubject.action",
        ...
        $.getJSON(path+"/esm/getESM.action", "examPaperId="+$(this).attr("id"), function(data) {
            //试卷中没有试题
            if (data == null || data == "") {
                zeroModal.show({
                    title : "试卷为空",
...
function removeSubjectWithExamPaper(subjectId, examPaperId, score) {
	...
	$.ajax({
		...
		url: path+"/esm/removeSubjectFromPaper.action",
		success: function() {
			//修改试卷行的试题数量和总分信息
