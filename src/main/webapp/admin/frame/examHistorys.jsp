					...
					<c:when test="${not empty historys }">
						<c:forEach items="${historys }" var="history">
							<tr>
								...
								<td>${history.examPaper.examPaperTime}</td>
								<td>${history.student.studentName}</td>
								<td>
									<div class="btn-group">
										<a href="${path}/Exam/review.action?studentId=${history.student.studentId}&examPaperId=${history.examPaper.examPaperId }&score=26&examPaperName=${history.examPaper.examPaperName }&studentName=${history.student.studentName }&examPaperTime=${history.examPaper.examPaperTime}" class="btn btn-info btn-sm">详情</a>
									</div>
								</td>
							</tr>
						</c:forEach>
