			...
			<thead>
				<tr>
					...
					<%--<c:if test="${sessionScope.adminPower == 1 }">--%>
						<th>操作
							&emsp;
							<a type="button" class="btn btn-xs btn-info" onclick="_iframe(2,'/course/toAdd.action')">添加</a>
					...
					<%--<c:when test="${not empty courselist }">--%>
						<c:forEach items="${courselist}" var="course">
							<tr>
								...
								<%--	<c:if test="${sessionScope.adminPower == 1 }">--%>
										<div class="btn-group">
											<button type="button" class="btn btn-info btn-sm" onclick="_iframe(1,'/course/selectByPrimaryKey.action?courseId=${course.courseId}')">修改</button>
											<button type="button" class="btn btn-danger btn-sm" onclick="del(${course.courseId },'/course/deleteCourseInfo.action')">删除</button>
										</div>
									<%--</c:if>--%>
								</td>
							</tr>
