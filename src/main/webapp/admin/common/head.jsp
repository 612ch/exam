...
<div style="width: 100%; height: 98px; background-color: #438EB9;">
	...
	<div id="loginTeacher" style="float:right; width: 190px; height:100%;line-height: 80px;">
		<%--<c:if test="${null!=((TeacherInfo)SecurityUtils.getSubject().getPrincipal()).getTeachername()}">--%>
		<%--<a target="right" class="btn btn-default" href="../teachers" style="margin-left: 9px;">
            &lt;%&ndash;${sessionScope.loginTeacher.teacherName }&ndash;%&gt;
            &lt;%&ndash;<%=((TeacherInfo)SecurityUtils.getSubject().getPrincipal()).getTeacherName() %>&ndash;%&gt;
            ${user.teacherName}
		...
		<%--</c:if>--%>
		<c:choose>
			<c:when test="${user.teacherName == null}">
				<a  class="btn btn-default" href="#" style="margin-left: 9px;">
					未登录
				</a>
			</c:when>
			<c:otherwise>
		...
		<a class="btn btn-default" href="${path}/teacher/goLogout.action" target="_parent">退出登录</a>
	</div>
</div>
